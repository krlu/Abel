...
object FinalProjectExperiment{
  ...
  private def ElGamalBob(p: Int, g: Int, b : Int, m : Int){
    val g_ab = FastExpWithMod(p)(788,142)
    println(s"Let p = $p , g = $g,  b = $b, m = $m ")
    println(s"141 ^ 142 mod 941 = ${FastExpWithMod(p)(g,b)}")
    println(s"788 ^ 142 mod 941 = $g_ab")
    println(s"m*(g^(ab)) = ${(m * g_ab) % 941}")
  }
}

trait Person
case object Alice extends Person
case object Bob extends Person
case object Eve extends Person
